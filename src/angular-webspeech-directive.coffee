'use strict'
angular.module('jonniespratley.angularWebspeechDirective', []).directive 'myDirective', ->
	restrict: 'EAC'
	scope: true
	link: compile = (tElement, tAttrs, transclude) ->
		Speech =
			startTimestamp: null
			recognizing: false
			recording: false
...
checkSpeech = ->
	if typeof speechRecognition isnt 'undefinded'
		new speechRecognition()
	else if typeof msSpeechRecognition isnt 'undefined'
		new msSpeechRecognition()
	else if typeof mozSpeechRecognition isnt 'undefined'
		new mozSpeechRecognition()
	else if typeof webkitSpeechRecognition isnt 'undefined'
